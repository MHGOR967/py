...
# === دوال جلب البيانات المتقدمة (محلياً بدون Countik) ===

def detect_language_from_bio(bio):
    """كشف لغة الحساب من البايو"""
    if not bio:
        return None
    import re as _re
    # عربي
    if _re.search(r'[\u0600-\u06FF\u0750-\u077F]', bio):
        return 'ar'
    # كوري
    if _re.search(r'[\uAC00-\uD7AF]', bio):
        return 'ko'
    # ياباني
    if _re.search(r'[\u3040-\u309F\u30A0-\u30FF]', bio):
        return 'ja'
    # صيني
    if _re.search(r'[\u4E00-\u9FFF]', bio):
        return 'zh'
    # هندي
    if _re.search(r'[\u0900-\u097F]', bio):
        return 'hi'
    # روسي
    if _re.search(r'[\u0400-\u04FF]', bio):
        return 'ru'
    # تركي (أحرف خاصة)
    if _re.search(r'[çşğıöüÇŞĞİÖÜ]', bio):
        return 'tr'
    # فرنسي
    if _re.search(r'[àâèéêëîïôùûüÀÂÈÉ]', bio):
        return 'fr'
    # إسباني
    if _re.search(r'[ñáéíóú¿¡Ñ]', bio):
        return 'es'
    # برتغالي
    if _re.search(r'[ãõçÃÕÇ]', bio):
        return 'pt'
    # إنجليزي (افتراضي إذا لاتيني)
    if _re.search(r'[a-zA-Z]', bio):
        return 'en'
    return None

def get_region_from_video(username, video_id):
    """جلب دولة الحساب من فيديو عبر tikwm (سريع ~3 ثواني)"""
    if not video_id:
        return None
    try:
        r = requests.get(
            f"https://www.tikwm.com/api/?url=https://www.tiktok.com/@{username}/video/{video_id}",
            timeout=10
        )
        if r.status_code == 200:
            data = r.json()
            if data.get("code") == 0 and data.get("data"):
                return data["data"].get("region")
    except:
        pass
    return None

def get_video_stats_from_tikwm(username, video_id):
    """جلب إحصائيات فيديو معين (مشاهدات، إعجابات، تعليقات، مشاركات)"""
    if not video_id:
        return None
    try:
        r = requests.get(
...
def calculate_analytics_local(followers, likes, videos_count):
    """حساب التحليلات محلياً من الإحصائيات الأساسية"""
    if videos_count <= 0 or followers <= 0:
        return None
    
    avg_likes = likes / videos_count
    # تقدير المشاهدات: عادة 5-7x الإعجابات على تيك توك
    est_avg_views = avg_likes * 5.5
    # معدل التفاعل
    likes_rate = (avg_likes / followers) * 100
    # تقدير التعليقات والمشاركات (نسب معروفة)
    est_comments_rate = likes_rate * 0.15  # التعليقات ~15% من الإعجابات
    est_shares_rate = likes_rate * 0.06  # المشاركات ~6% من الإعجابات
    total_rate = likes_rate + est_comments_rate + est_shares_rate
    
    est_avg_comments = int(avg_likes * 0.15)
    est_avg_shares = int(avg_likes * 0.06)
    
    # تقدير الأرباح: $0.02-$0.05 لكل 1000 مشاهدة
    total_views = est_avg_views * videos_count
    earnings_min = (total_views / 1000) * 0.02
    earnings_max = (total_views / 1000) * 0.05
    
    return {
        "earnings": {"min": round(earnings_min, 2), "max": round(earnings_max, 2)},
        "engagement_rates": {
            "likes_rate": round(likes_rate, 2),
            "comments_rate": round(est_comments_rate, 2),
            "shares_rate": round(est_shares_rate, 2),
            "total_rate": round(total_rate, 2),
        },
        "performance": {
            "avgViews": int(est_avg_views),
            "avgLikes": int(avg_likes),
            "avgComments": est_avg_comments,
            "avgShares": est_avg_shares,
        },
    }

def get_countik_userinfo(sec_uid):
    """محاولة جلب بيانات countik (إذا اشتغل - بونس)"""
    try:
        headers = {
        ...
        r = requests.get(f"https://countik.com/api/userinfo?sec_user_id={sec_uid}", headers=headers, timeout=5)
        if r.status_code == 200:
            return r.json()
    except:
        pass
...
def get_countik_analyze(sec_uid):
    """محاولة جلب تحليل countik (إذا اشتغل - بونس)"""
    try:
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36',
            'Accept': 'application/json, text/plain, */*',
        ...
        r = requests.get(f"https://countik.com/api/analyze?sec_user_id={sec_uid}", headers=headers, timeout=5)
        if r.status_code == 200:
            return r.json()
    except:
        pass
...
def get_region_from_videos(username, video_id=None):
    """جلب دولة الحساب من الفيديوهات عبر tikwm (سريع)"""
    return get_region_from_video(username, video_id)
...
def handle_callback(call):
    ...
    # === أزرار البحث ===
        ...
        # محاولة countik (سريعة)
        analyze_data = get_countik_analyze(sec_uid) if sec_uid else None
        
        # حساب محلي كاحتياطي
        local_analytics = calculate_analytics_local(followers, likes, videos_count)
        
        # استخدام countik إذا اشتغل، وإلا الحساب المحلي
        if analyze_data:
            earnings = analyze_data.get("earnings")
            engagement = analyze_data.get("engagement_rates")
            performance = analyze_data.get("performance")
            hashtags_data = analyze_data.get("hashtags", [])
        else:
            earnings = local_analytics.get("earnings") if local_analytics else None
            engagement = local_analytics.get("engagement_rates") if local_analytics else None
            performance = local_analytics.get("performance") if local_analytics else None
            hashtags_data = []
...
def handle_message(message):
    ...
    # محاولة countik (سريعة بدون انتظار طويل)
...
# === بروكسي لتجاوز حجب Cloudflare ===
PROXY_URLS = [
    "https://api.allorigins.win/raw?url={}",
]

def proxy_get(url, timeout=20):
    """جلب البيانات عبر بروكسي مجاني لتجاوز Cloudflare"""
    # محاولة مباشرة أولاً
    try:
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36',
    ...
    for proxy_template in PROXY_URLS:
        try:
            proxy_url = proxy_template.format(quote(url))
...
def get_countik_userinfo(sec_uid):
    ...
    return proxy_get(url, timeout=20)

def get_countik_analyze(sec_uid):
    url = f"https://countik.com/api/analyze?sec_user_id={sec_uid}"
    return proxy_get(url, timeout=25)

def get_region_from_videos(username, video_id=None):
    """جلب دولة الحساب من الفيديوهات"""
    # محاولة 1: عبر tikwm API مباشرة
    if video_id:
        try:
            ...
            pass
    # محاولة 2: عبر tikwm مع بروكسي allorigins
    if video_id:
        try:
            target = f"https://www.tikwm.com/api/?url=https://www.tiktok.com/@{username}/video/{video_id}"
            data = proxy_get(target, timeout=20)
            if data and data.get("code") == 0 and data.get("data"):
                region = data["data"].get("region")
                if region:
                    return region
...
def handle_message(message):
    ...
    if analyze_data and analyze_data.get("videos"):
        video_id_for_region = analyze_data["videos"][0].get("id")
    # fallback: محاولة جلب video_id من tikwm عبر البروكسي إذا countik ما رجع شي
    if not video_id_for_region:
        try:
            posts_url = f"https://www.tikwm.com/api/user/posts?unique_id={unique_id}&count=1"
            posts_data = proxy_get(posts_url, timeout=20)
            if posts_data and posts_data.get("code") == 0 and posts_data.get("data") and posts_data["data"].get("videos"):
